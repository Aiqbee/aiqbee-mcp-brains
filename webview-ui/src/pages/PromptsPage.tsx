...
const prompts: Prompt[] = [
  ...
  {
    label: 'Migrate CLAUDE.md to a development brain',
    text: 'I want to move project knowledge out of my CLAUDE.md file and into the <Brain Name> brain so it can be shared, versioned, and queried by any AI assistant via MCP. Follow these steps:\n\n1. Read the current CLAUDE.md (or equivalent project instructions file) in this repository.\n2. For each distinct topic (coding standards, architecture decisions, development practices, common patterns, lessons learned, environment setup, CI/CD rules, review checklists), create a neuron in the <Brain Name> brain with the appropriate neuron type.\n3. Create synapses (relationships) between related neurons — e.g. link a coding standard to the lesson learned that motivated it.\n4. After all neurons are created, replace the CLAUDE.md with a slim version that only contains: (a) the product name, (b) a one-line description, (c) essential build/run commands, and (d) an instruction to search the <Brain Name> brain before starting any task.\n5. Show me a summary of neurons created, relationships added, and the proposed slim CLAUDE.md before applying changes.',
  },
];
